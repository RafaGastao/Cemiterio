...
def financeiro_collection():
    ...
    if request.method == 'GET':
        try:
            cur.execute('SELECT id, tipo_transacao, descricao, valor, data FROM financeiro')
            data = cur.fetchall()
            return jsonify(data)
        except Exception as e:
    ...
    if request.method == 'POST':
        ...
        try:
            cur.execute(
                'INSERT INTO financeiro (tipo_transacao, descricao, valor, data) VALUES (%s, %s, %s, %s) RETURNING id',
...
def financeiro_single(fid):
    ...
    try:
        if request.method == 'GET':
            cur.execute('SELECT id, tipo_transacao, descricao, valor, data FROM financeiro WHERE id=%s', (fid,))
            row = cur.fetchone();
